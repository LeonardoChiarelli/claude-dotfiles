...
- Plugin manifest (`plugins.json`): marketplaces + installed plugins, for reinstall via `claude` CLI
...
├── plugins.json         # marketplaces + plugins manifest (generated)
...
- **`plugins.json`**: generated by sync from `~/.claude/plugins/known_marketplaces.json` + `installed_plugins.json` (names/repos only, never cache contents). Install loops `claude plugin marketplace add <repo>` then `claude plugin install <plugin>@<marketplace>`. If the CLI doesn't support headless install, print manual instructions as fallback.
